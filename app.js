class CarQuestApp {
    ...
    async getAIResponse(query) {
        try {
            ...
            const isPriceQuery = (query.toLowerCase().includes('price') || 
                                 query.toLowerCase().includes('cost') || 
                                 query.toLowerCase().includes('how much')) &&
                                (query.toLowerCase().startsWith('what is the price of') ||
                                 query.toLowerCase().startsWith('what\'s the price of') ||
                                 query.toLowerCase().startsWith('tell me the price of') ||
                                 query.toLowerCase().startsWith('show me the price of') ||
                                 query.toLowerCase().startsWith('price of') ||
                                 query.toLowerCase().startsWith('cost of') ||
                                 query.toLowerCase().startsWith('how much is') ||
                                 query.toLowerCase().startsWith('how much does it cost'));
            ...
            if (isComparisonQuery) {
                ...
                // Extract car details from the price query
                const carDetails = query.replace(/price|cost|how much|what is the|what's the|tell me the|show me the|price of|cost of|how much is|how much does it cost/gi, '').trim();
                systemPrompt = `You are a precise automotive pricing assistant. Provide current and accurate pricing information for the ${carDetails} in this exact format. Only include prices that are confirmed and up-to-date:\n\n## New Car Pricing\nBase Price: [price]\nTop Trim Price: [price]\n\n## Used Car Pricing\n1 Year Old: [price]\n3 Years Old: [price]\n5 Years Old: [price]\n\n## Additional Costs\nDestination Fee: [price]\nTaxes & Fees: [price]\nRegistration: [price]`;
            } else {
...
- [Additional historical information]`;
    ...
    formatResponse(response) {
        ...
        sections.forEach(section => {
            ...
            if (!this.isPriceQuery && (section.includes('Pricing') || section.includes('Cost') || section.includes('Price'))) return;
    ...
    formatTableValue(value) {
        // Format price ranges (e.g., $20,000 - $30,000)
        if (value.match(/\$[\d,]+k?\s*-\s*\$[\d,]+k?/)) {
            return `<span class="price-range">${value}</span>`;
        }
        
        // Format single prices (e.g., $50,000, $100k)
        if (value.match(/\$[\d,]+k?/)) {
